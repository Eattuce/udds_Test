...
/* 发布者函数 */
extern "C" int publisher_main(int domainId, int sample_count, const unsigned long data_size)
{
	...
	for (count = 0; (sample_count == 0) || (count < sample_count); ++count) {
		instance->id = 11;
		char* value = nullptr;
		// 根据数据量大小创建字符数组，并设置 value 字段的值
		unsigned long valueSize = data_size - sizeof(instance->id);
		if (valueSize > 0) {
			value = new char[valueSize];
			memset(value, 'a', valueSize); // 填充字符数组
		...
		demo_instance->data = value;
		std::string str = value;
		const char *mdval = GetDataMD5(str).data();
		strcpy(demo_instance->mdval, mdval);
...
int main(int argc, char *argv[])
{
	...
	if (argc >= 4) {
		data_size = atoi(argv[3]); /* 发送sample_count次 */
	}
